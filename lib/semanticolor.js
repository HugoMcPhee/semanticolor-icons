...
let grammarList = [];
try {
	grammarList = Sugar.Array.compact(
		fs.readFileSync(grammarListFile, { encoding: 'utf-8' }).split(/\r\n|\r|\n/),
		true
	);
} catch (e) {
	grammarList = [];
...
let Semanticolor = {
	...
	activate: function () {
		...
		function createGrammar(grammar) {
			...
			if (options) {
				grammars[paramName] = SemanticolorGrammarFactory.create(
					grammar,
					defaults,
					options,
		...
		function isSupportedGrammar(grammar) {
			let result = false;
			if (
				grammar.packageName !== SemanticolorGrammarFactory.packageName &&
