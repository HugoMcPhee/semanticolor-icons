...
function handleColoringQuotedStrings(passedInEditor) {
	...
	// (NOTE markerLayersByEditorId editors never get removed, so it might be a memory leak at the moment?)
...
function traverseTree(tree) {
	...
	while (reached_root == false) {
		if (cursor.currentNode.type === 'string') {
			foundRanges.push(cursor.currentNode.range);
		}
...
function checkIfRangeAlreadyHasMarker(range, editor, markerLayer) {
	...
	for (let i = 0; i < markers.length; i++) {
		...
		if (range.start.isEqual(marker.getStartBufferPosition())) {
			if (range.end.isEqual(marker.getEndBufferPosition())) {
...
function handleColoringQuotedStringsInEditor(editor, cfg) {
	...
	const foundRanges = traverseTree(treeSitterTree);

	const textBuffer = editor.getBuffer();
	const markerLayer = markerLayersByEditorId[editor.id];

	for (let i = 0; i < foundRanges.length; i++) {
		const range = foundRanges[i];
		...
		if (!checkIfRangeAlreadyHasMarker(newRange, editor, markerLayer)) {
			...
			const foundText = textBuffer.getTextInRange(newRange);

			editor.decorateMarker(marker, {
				type: 'text',
				class: getColoredClassName(foundText, cfg),
			});
		}
	}
}
