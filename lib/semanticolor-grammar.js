...
const packageName = 'semanticolor-icons';
			...
			switch (action) {
				...
				case 'colorize-bg':
					...
					if (
						...
						defaultAction === 'colorize-bg'
					) {
						let token = str.trim();
						if (symbolPrefix.test(token)) {
							token = token.substring(1);
						}
						// NOTE This part is different to the original semanticolor, it adds the original scope to the end
						newScope = semanticolorScope(
							token,
							colorDiversity,
							action === 'colorize-bg'
						);
						newScope += addIconsToScope(token);
						newScope += ` syntax--${startScope}`;
					} else {
						newScope = getNewScope(
							startScope,
							str,
